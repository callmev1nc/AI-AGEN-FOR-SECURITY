...
export function detectDangerousPatternsInFiles(files: FileInput[]): VulnerabilityResult[] {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (dangerous.regex.test(line)) {
          const existing = findings.find(
            (f) => f.title === dangerous.title && f.filePath === file.path
          );
          if (!existing) {
            findings.push({
              ...
              description: `Found in ${file.path} at line ${i + 1}: ${dangerous.description}`,
