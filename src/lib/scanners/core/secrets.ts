import type { FileInput, VulnerabilityResult } from "./types";
...
export function findLineNumber(content: string, match: string): number {
...
export function getContextLines(content: string, lineNumber: number, before = 3, after = 2): string {
  const lines = content.split("\n");
...
export function detectSecretsInFiles(files: FileInput[]): VulnerabilityResult[] {
  ...
  for (const file of files) {
    for (const pattern of SECRET_PATTERNS) {
      ...
      const lineIndex = findLineNumber(file.content, matches[0]);
      const context = getContextLines(file.content, lineIndex);
      ...
      findings.push({
        ...
        title: `${pattern.name} detected in ${file.path}`,
        description: `A ${pattern.name} was found in "${file.path}". Hardcoded secrets in source code can lead to account compromise, data breaches, and unauthorized access.`,
        evidence: `File: ${file.path}, Line: ${lineIndex}\nPattern: ${maskedMatch}\nContext:\n${context}`,
        remediation: pattern.remediation,
        cvssScore: pattern.severity === "critical" ? 9.0 : pattern.severity === "high" ? 7.0 : 3.0,
        affectedUrl: `file://${file.path}`,
        filePath: file.path,
        lineStart: lineIndex,
      });
    }
