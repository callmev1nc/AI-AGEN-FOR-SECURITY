...
export const CVE_PATTERNS: Array<{
  name: string;
  package: string;
...
function checkPackageJson(content: string): VulnerabilityResult[] {
        ...
        if (pkg === cve.package && cve.versionRange(cleanVer)) {
          ...
          if (!existing) {
            findings.push({
              ...
              evidence: `Found ${pkg}@${cleanVer} in package.json`,
              remediation: cve.remediation,
              cvssScore: cve.severity === "high" ? 7.5 : cve.severity === "medium" ? 5.0 : 3.0,
              affectedUrl: "file://package.json",
...
function checkRequirementsTxt(content: string): VulnerabilityResult[] {
  const findings: VulnerabilityResult[] = [];
...
export function detectVulnerableDepsInFiles(files: FileInput[]): VulnerabilityResult[] {
  const findings: VulnerabilityResult[] = [];

  for (const file of files) {
    if (file.path === "package.json" || file.path.endsWith("/package.json")) {
      findings.push(...checkPackageJson(file.content));
    }
    if (file.path === "requirements.txt" || file.path.endsWith("/requirements.txt")) {
      findings.push(...checkRequirementsTxt(file.content));
    }
  }
