...
export default function ScanResultsPage() {
  ...
  if (scan.status === "queued" || scan.status === "running") {
    return (
      <div className="animate-fade-in-up flex min-h-[50vh] items-center justify-center">
        <div className="text-center">
          <div className="mx-auto mb-4 h-12 w-12 animate-spin rounded-full border-2 border-[var(--accent-dim)] border-t-[var(--accent)]" />
          ...
          <p className="mt-2 text-sm text-[var(--text-secondary)]">
            Scanning {scan.targetUrl}...
          </p>
          <button
            onClick={() => window.location.reload()}
            className="mt-4 rounded-lg bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-black hover:brightness-110"
          >
            Refresh
          </button>
        </div>
        ...
        <div className="card-base col-span-2 flex items-center gap-6 p-6">
          <div className="relative flex h-24 w-24 shrink-0 items-center justify-center">
            ...
            <div className="absolute flex flex-col items-center">
              <span className="text-2xl font-bold" style={{ color: scoreColor, fontFamily: "var(--font-jetbrains)" }}>
                {score}
              </span>
            </div>
          </div>
                  ...
                  <summary className="flex items-center gap-4 p-4 list-none [&::-webkit-details-marker]:hidden">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-3">
                        ...
                        <span
                          className="shrink-0 rounded border border-[var(--border)] px-2 py-0.5 text-[10px] text-[var(--text-muted)] uppercase tracking-wider"
                          style={{ fontFamily: "var(--font-jetbrains)" }}
                        >
                          {vuln.category}
                        </span>
